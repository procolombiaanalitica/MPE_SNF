...
columnas_int = [columnas[5],columnas[6],columnas[7],columnas[13],columnas[14],columnas[11],columnas[10],columnas[15],columnas[16]]
columnas_float = [columnas[8],columnas[9]]
tds = {col: float for col in columnas_float}
tds2 = {col: int for col in columnas_int} 
dtype = {**tds, **tds2}
...
columnas = base2.columns.to_list()
...
prod_pm=A.prod_pm.set_index("Pais")

sec_pc= A.sec_pc.set_index("Pais")
sec_pm=A.sec_pm.set_index("Pais")

sub_pc=A.sub_pc.set_index("Pais")
sub_pm= A.sub_pm.set_index("Pais")
...
#productos
prod_pm2=A.prod_pm2.set_index("Posición")
prod_pc2=A.prod_pc2.set_index("Posición")

#Sectores
sec_pm2=A.sec_pm2.set_index("Sector")
sec_pc2=A.sec_pc2.set_index("Sector")
  #Subsectores
sub_pm2=A.sub_pm2.set_index("Subsector")
sub_pc2=A.sub_pc2.set_index("Subsector")
...
def make_puntajes(tab,value):
    try:
        pc=dict_puntajes[tab][0][value].mean(axis=1)
    ...
    mean["PF"]=norm_serie(mean.mean(axis=1))
    mean=mean.round(4)
...
def update(tab,cadena):
    if cadena==None:
        options=dict_drops[tab]['options']
        val=dict_drops[tab]['value']
    else:
    ...
    return dict_titles[tab],options,dict_drops[tab]['multi'],dict_drops[tab]['placeholder'],val
...
def update_table(tab,value,check):
    ...
    filt["Ventaja Comparativa Revelada"]=(filt["Total Exportaciones 2018-2022 (miles USD)"]/filt["Suma Exportaciones Total"])/(
        filt["Total Importaciones 2018-2022 (miles USD)"]/filt["Suma Importaciones Total"])

    filt["Ventaja Comparativa Revelada"]=filt["Ventaja Comparativa Revelada"].fillna(0).round(2)
    ...
    del filt,v
    return [{'name':c,'id':c,'type':dict_formatos[c][0],
        'format':dict_formatos[c][1]} for c in mean.columns],mean.to_dict("records")
...
def update_map(drop,tab,value):
    ...
    try:
        #Base filtrada
        if tab=="t1":
            filt=base[cols_to_use][base["Sector"].isin([drop])].groupby("Pais").sum()
            filt=filt.reset_index()
            title="{} de {}".format(value,drop)
            """    
            filt=base.groupby(["Pais",dict_base[tab]]).agg(dict_agg)
            filt=filt.reset_index(["Pais"]).loc[drop]
            title="{} de {}".format(value,drop)
            """
        elif tab=="t2":
            filt=base[cols_to_use][base["Subsector"].isin(drop)].groupby("Pais").sum()
            filt=filt.reset_index()
            if len(drop)==1:
                title="{} de {}".format(value,drop[0])
            else:
                title="{} de los subsectores seleccionados".format(value)
        else:
            filt=base[cols_to_use][base["Posición"].isin(drop)].groupby("Pais").sum()
            filt=filt.reset_index()
            if len(drop)==1:
                title="{} de la posición {}".format(value,drop[0])
            else:
                title="{} de los productos seleccionados".format(value)

        #print(filt.columns)
        # #Traer longitud y latitud        
        filt=filt.merge(corr,on="Pais")
        ...
        if filt[value].sum()!=0:
            fig = go.Figure([
                ...
                text="<a style='color:white;' href='https://fichas-pais.herokuapp.com/" +
                    filt["Pais"].str.replace(" ","").str.lower().str.replace("á","a").str.replace("é","e").str.replace("í","i").str.replace("ó","o").str.replace("ú","u")+
                    "'>"+filt["Pais"]+"<br>"+ str(value) +
                    ": " +filt[value].apply(addComa),
                hoverinfo='text',
